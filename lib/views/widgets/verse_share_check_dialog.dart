...
class VerseShareCheckDialog extends StatelessWidget {
  const VerseShareCheckDialog({super.key});
            ...
            children: [
              Expanded(
                child: AppButton(
                  ...
                  onTap: () {
                    context.pop();
                    context.read<HomeCubit>().checkBibleVerseChallenge(false);
                    UserModel? user = preferences.getUserModel();
                    if (user != null) {
                      user = user.copyWith(isBibleVerse: 0, isChallenge: 0);
                      preferences.saveUserModel(user);
                    }
                    context.read<HomeCubit>().getBibleVerse(true);
                    context
              ...
              Expanded(
                child: AppButton(
                  ...
                  onTap: () {
                    context.pop();
                    context.read<HomeCubit>().checkBibleVerseChallenge(true);
                    UserModel? user = preferences.getUserModel();
                    if (user != null) {
                      user = user.copyWith(isBibleVerse: 0);
                      preferences.saveUserModel(user);
                    }
                    context.read<HomeCubit>().getBibleVerse(true);
                    // context.pop();
